...
import { setIsReviewPopupOpen, setAverageRating, setSelectedReview, setReviews } from '../Slices/reviewSlice';

const OverviewPopup = ({ doctorData, onClose }) => {
  const dispatch = useDispatch();
  const isMobileScreen = useSelector(state => state.navbar.isMobileScreen);
  const { reviews, averageRating, selectedReview, isReviewPopupOpen } = useSelector(state => state.review);
  ...
  const handleReviewSave = (reviewData) => {
    if (doctorData.id) {
      ...
      dispatch(setReviews(existingReviews));
      dispatch(setAverageRating(calculateAverageRating(existingReviews)));
  ...
  useEffect(() => {
    ...
    dispatch(setAverageRating(calculateAverageRating(fetchedReviews)));
  }, [doctorData]);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-blur" dir='rtl'>
      <div className={`bg-white rounded-lg shadow-lg overflow-scroll ${isMobileScreen ? 'min-h-[600px] max-h-[600px]' : 'w-full max-w-lg'}`} style={{ width: isMobileScreen ? "300px" : "" }}>
        <div className="p-6 text-center overflow-scroll">
          ...
          <div className="flex justify-center">
            <StarRating rating={averageRating} onRatingChange={false} readOnly />
          </div>
          {user !== null && (
