...
const OverviewPopup = ({ doctorData, onClose }) => {
  ...
  const NotesCellRenderer = ({ value }) => {
    ...
    return (
      <div>
        <button onClick={handleShowPopup} className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700" title='showbtn'>Show</button>
      </div>
    );
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-blur">
      <div className="bg-white rounded-lg shadow-lg overflow-hidden w-full max-w-lg">
        <div className="p-6 text-center">
          <h2 className="text-2xl font-semibold mb-2">Doctor Overview</h2>
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mb-4 justify-center">
            <p className="flex-grow">Doctor Specialty: {doctorData.major}</p>
            <p className="flex-grow">adress:{doctorData.address}</p>
          </div>
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mb-4 justify-center">
          ...
          {isLoging && (
            <button className="mt-4 py-2 px-4 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition duration-300" onClick={handleReviewPopupOpen} title='ratingbtn'>rate</button>
          )}
        </div>
